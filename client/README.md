...
- [x] Explain what a token is used for.
      JSON Web Tokens, or JWTs are strings of cryptic text which are returned from a server if it has a login endpoint and recieves a payload of username and password. After the server returns the token, it can then be used to access protected routes and other authentication duties by adding an Authorization : "token" to the headers of future HTTP requests.

- [x] What steps can you take in your web apps to keep your data secure?
      In addition to Authenticating API requests, you can set up protected routes that only render when the proper authentication is present.

- [x] Describe how web servers work.
      Web servers process incoming network requests sent over HTTP/ various other related protocols. Their primary function is to store, process, and deliver web pages go clients. Communication between client and server takes place using Hypertext Transfer Protocol (HTTP)

- [x] Which HTTP methods can be mapped to the CRUD acronym that we use when interfacing with APIs/Servers.
      CRUD stands for Create, Read, Update, and Delete. The various HTTP methods that correspond to these attributes are as follows:

  Create - .POST requests - Used to request that the origin server accept the entity enclosed in the request as a new subordinate of the resourde identified by the Request-URI in the Request-Line

  Read - .GET requests - Requests a representation of the specified resource

  Update - .PUT or .PATCH requests - Used to change a a resouce's information. PUT takes in a body object and identifies data that needs to be updated.

  Delete - .DELETE requests - Pretty self explanatory. Used to delete a route, with similar implementation to a .POST or .PUT reauest.
